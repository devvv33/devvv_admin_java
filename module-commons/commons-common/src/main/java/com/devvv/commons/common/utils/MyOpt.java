...
import com.devvv.commons.common.utils.function.ThrowingFunction;
 ...
 * 拷贝自hutool包的 {@link cn.hutool.core.lang.Opt}, 新增改动:
 *  1. 添加了{@link #ifFailThrow} 方法
 *  2. 添加了{@link #tryMap} 方法
 */
public class MyOpt<T> {
    /**
     * 一个空的{@code Opt}
     */
    private static final MyOpt<?> EMPTY = new MyOpt<>(null);
    ...
    public static <T> MyOpt<T> empty() {
        return (MyOpt<T>) EMPTY;
    }
     ...
     */
    public static <T> MyOpt<T> of(T value) {
        return new MyOpt<>(Objects.requireNonNull(value));
    }
     ...
     */
    public static <T> MyOpt<T> ofNullable(T value) {
     ...
     */
    public static <T> MyOpt<T> ofBlankAble(T value) {
        return StrUtil.isBlankIfStr(value) ? empty() : new MyOpt<>(value);
    }
     ...
     * @since 5.7.17
     */
    public static <T, R extends Collection<T>> MyOpt<R> ofEmptyAble(R value) {
        return ObjectUtil.isEmpty(value) ? empty() : new MyOpt<>(value);
    }
     ...
     * @return 操作执行后的值
     */
    public static <T> MyOpt<T> ofTry(Func0<T> supplier) {
        try {
            return MyOpt.ofNullable(supplier.call());
        } catch (Exception e) {
            final MyOpt<T> empty = new MyOpt<>(null);
            empty.exception = e;
            return empty;
        }
    }
     ...
     */
    public MyOpt<T> ifFail(final Consumer<? super Throwable> action) throws NullPointerException {
    ...
    public MyOpt<T> ifFailThrow(Function<Throwable, ? extends RuntimeException> exceptionFun) {
        if (isFail()) {
            throw exceptionFun.apply(this.exception);
        }
        return this;
    }
     ...
     * @throws NullPointerException 如果包裹里的值存在，但你传入的操作为{@code null}时抛出
     */
    public MyOpt<T> ifPresent(Consumer<? super T> action) {
        if (isPresent()) {
            action.accept(value);
        }
        return this;
     ...
     */
    public MyOpt<T> ifPresentOrElse(Consumer<? super T> action, VoidFunc0 emptyAction) {
        if (isPresent()) {
            action.accept(value);
        } else {
     ...
     * @throws NullPointerException 如果包裹里的值存在时，执行的操作为 {@code null}, 或者包裹里的值不存在时的操作为 {@code null}，则抛出{@code NPE}
     */
    public <U> MyOpt<U> mapOrElse(Function<? super T, ? extends U> mapper, VoidFunc0 emptyAction) {
        if (isPresent()) {
     ...
     */
    public MyOpt<T> filter(Predicate<? super T> predicate) {
     ...
     */
    public <U> MyOpt<U> map(Function<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
        if (isEmpty()) {
            return empty();
        } else {
            return MyOpt.ofNullable(mapper.apply(value));
        }
    }

    public <U> MyOpt<U> tryMap(ThrowingFunction<? super T, ? extends U> mapper) {
        Objects.requireNonNull(mapper);
        if (isEmpty()) {
            return empty();
        }
        else {
            return MyOpt.ofTry(()->mapper.apply(value));
        }
    }
     ...
     * 如果不存在，返回一个空的{@code Opt}
     * 和 {@link MyOpt#map}的区别为 传入的操作返回值必须为 Opt
     *
     * @param mapper 值存在时执行的操作
     * @param <U>    操作返回值的类型
     ...
     */
    public <U> MyOpt<U> flatMap(Function<? super T, ? extends MyOpt<? extends U>> mapper) {
        Objects.requireNonNull(mapper);
        if (isEmpty()) {
            return empty();
        } else {
            @SuppressWarnings("unchecked")
            final MyOpt<U> r = (MyOpt<U>) mapper.apply(value);
     ...
     * 如果包裹里的值存在，就执行传入的操作({@link Function#apply})并返回该操作返回值
     * 如果不存在，返回一个空的{@code Opt}
     * 和 {@link MyOpt#map}的区别为 传入的操作返回值必须为 {@link Optional}
     *
     * @param mapper 值存在时执行的操作
     * @param <U>    操作返回值的类型
     * @return 如果包裹里的值存在，就执行传入的操作({@link Function#apply})并返回该操作返回值
     ...
     * @since 5.7.16
     */
    public <U> MyOpt<U> flattedMap(Function<? super T, ? extends Optional<? extends U>> mapper) {
        Objects.requireNonNull(mapper);
        if (isEmpty()) {
            return empty();
        } else {
     ...
     * @author VampireAchao
     */
    public MyOpt<T> peek(Consumer<T> action) throws NullPointerException {
     ...
     */
    @SafeVarargs
    public final MyOpt<T> peeks(Consumer<T>... actions) throws NullPointerException {
        // 第三个参数 (opts, opt) -> null其实并不会执行到该函数式接口所以直接返回了个null
        return Stream.of(actions).reduce(this, MyOpt<T>::peek, (opts, opt) -> null);
    }
     ...
     */
    public MyOpt<T> or(Supplier<? extends MyOpt<? extends T>> supplier) {
        Objects.requireNonNull(supplier);
        if (isPresent()) {
            return this;
        } else {
            @SuppressWarnings("unchecked") final MyOpt<T> r = (MyOpt<T>) supplier.get();
            return Objects.requireNonNull(r);
        }
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof MyOpt)) {
            return false;
        }

        final MyOpt<?> other = (MyOpt<?>) obj;
