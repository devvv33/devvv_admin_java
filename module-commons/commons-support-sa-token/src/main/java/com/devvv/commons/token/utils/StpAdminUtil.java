package com.devvv.commons.token.utils;
 ...
 *
 * admin 用户管理类
 * 拷贝自 {@link cn.dev33.satoken.stp.StpUtil}，这里仅改了 TYPE = "admin"
 * 改动：
 *  1. 将 {@link StpAdminUtil#TYPE} 固定设置为 TYPE = "admin"
 *  2. 将 {@link #getLoginId()} 方法改为返回Long类型
 *
 * 用法参考：
 *  https://sa-token.cc/doc.html#/up/many-account
 *  我们采用: 不同账号体系，用不同的工具类来操作
 *  此工具类操作CMS的admin账号
 */
public class StpAdminUtil {
     ...
     * @return 账号id
     */
    // mark: WangSJ 本系统中，用户ID都是Long类型，所以此处直接返回Long
