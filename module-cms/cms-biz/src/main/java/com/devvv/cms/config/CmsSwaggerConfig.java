...
public class CmsSwaggerConfig {
    ...
    public GroupedOpenApi createApi() {
        return GroupedOpenApi.builder()
                ...
                .displayName("1-CMS接口")
                .pathsToMatch("/cmsApi/**")
                .packagesToScan("com.devvv.cms.controller")
                .addOperationCustomizer((operation, handlerMethod) -> {
                    // 设置默认的 请求头信息值
                    operation.setParameters(List.of(
                            new Parameter().in(ParameterIn.HEADER.toString()).schema(new StringSchema()).name(StpAdminUtil.getTokenName()).required(false), // 这里设置为false，仅表示在swagger中无需手动传递（因为有cookie），在app中是必须要传递的
